# Post-mortem: the alignment bugs behind Sharpe 1.56
...
Four alignment bugs compounded. Two of them (1 and 2) were identified in
the April 2026 bug report; the other two surfaced only once the first two
were fixed, because the original code structurally could not reach the
data that exposed them.
...
| Metric | Published (buggy) | Corrected |
|--------|-------------------|-----------|
| Sharpe | 1.56 | 1.10 |
| Sortino | 2.85 | 1.63 |
| Deflated SR | 0.97 | 0.76 |
| Max DD | 3.6% | 2.6% |
| Hit rate | 55.0% | 65.0% |
| Trades | 20 | 32 |
| Days in position | — | 40 of 172 |

**Do not read the corrected Sharpe as evidence of edge.** The diagnostics
from the same run say there is none:

- Stage-2 OOS correlation **+0.0052**, OOS R² **−0.0011** — worse than
  predicting the mean.
- ElasticNet weights are ~0 on everything (OrderFlow 0.033, Sentiment
  0.052, VannaCharm / VRP / Macro exactly 0).
- The P&L comes from 40 active days. That is not a sample you can draw a
  conclusion from.

In other words the same intercept-drift mechanism that produced 1.56 is
still what produces 1.10; the bugs are gone, but the strategy has not been
shown to predict CL direction. This matches the April 2026 bug report's
...
- **Full-sample standardization.** `build_vpin_features`,
  `build_kyle_features` and `build_hawkes_features` z-score using the mean
  and std of the entire sample, which leaks future information into every
  bar. Not fixed here; it is a separate bug from the alignment work.
- **`backtesting/test_pipeline.py` does not run** — it imports a module
  `lob_kernel` that does not exist. It was already broken before this
  work, so the "11 synthetic end-to-end tests" in the old README notes
  were not running either. `test_feature_alignment.py` (15 tests) is what
  currently passes.
